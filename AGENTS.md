...
`vibly-console` is the human-facing Web UI for the Vibly coordinator network. It does **not** define HTTP contracts — those come from `@vibly-ai/coordinator-http-contract`, which derives types from `vibly-coordinator`'s Fastify routes.

## Invariants

1. **Do not write new handwritten Coordinator paths.** `scripts/check-handwritten-paths.mjs` scans all of `src/` except `src/lib/coordinator/contractClient.ts` (the proxy/transport adapter). Any other place that emits literals like `fetch("/projects/...")` or `new EventSource("/projects/...")` will fail `pnpm lint`. New code must consume `@vibly-ai/coordinator-http-contract`.
