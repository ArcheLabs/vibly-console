...
export function invalidateForEvent(queryClient: QueryClient, projectId: string, event: EventEnvelope, networkId: string): void {
  const type = event.type;
  if (type === "ProjectCreated") void queryClient.invalidateQueries({ queryKey: queryKeys.projects(networkId) });
  ...
  if (type.includes("Trace")) {
    void queryClient.invalidateQueries({ queryKey: queryKeys.traces(networkId) });
  ...
  if (type.includes("PhaseGTimeline")) void queryClient.invalidateQueries({ queryKey: queryKeys.projectTimeline(networkId, projectId) });
  ...
  void queryClient.invalidateQueries({ queryKey: queryKeys.projectOverview(networkId, projectId) });
  ...
  void queryClient.invalidateQueries({ queryKey: queryKeys.events(networkId, projectId) });
}
